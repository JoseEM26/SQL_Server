...
SELECT ID_CLIENTE,NOMBRES,APELLIDOS,DIRECCION
FROM CLIENTE
GO
...
SELECT
	Código			= PRO.ID_PRODUCTO,
	Descripción		= PRO.DESCRIPCION,
	[Stock actual]	= PRO.STOCK_ACTUAL,
	[Stock mínimo]	= PRO.STOCK_MINIMO
...
SELECT *
FROM PRODUCTO AS PRO
WHERE PRO.ID_PRODUCTO IN ('PRO002','PRO004')
...
WHERE PRO.PRECIO_VENTA BETWEEN 30 AND 100
GO
...
WHERE CLI.ID_DISTRITO NOT IN('D003','D010')
GO
...
WHERE EMP.NOMBRES LIKE 'R%' AND EMP.APELLIDOS LIKE 'T%'
GO
...
SELECT * FROM EMPLEADO AS EMP
WHERE EMP.APELLIDOS LIKE '_A%' AND EMP.ESTADO_CIVIL = 'S'
GO
...
SELECT
	PRO.ID_PRODUCTO, PRO.DESCRIPCION, PRO.PRECIO_VENTA, PRO.STOCK_ACTUAL
FROM PRODUCTO AS PRO
WHERE PRO.DESCRIPCION LIKE '[AM]%' AND PRO.PRECIO_VENTA >= 15 AND PRO.STOCK_ACTUAL >= 30
GO
...
SELECT
	EMP.COD_EMPLE, EMP.NOMBRES, EMP.APELLIDOS, EMP.ESTADO_CIVIL, 
	EMP.NIVEL_EDUCA,EMP.EMAIL, EMP.SUELDO_BASICO
FROM EMPLEADO AS EMP
WHERE EMP.ESTADO_CIVIL = 'C' AND EMP.NIVEL_EDUCA IN ('SUPERIOR','SECUNDARIA') 
	AND EMP.SUELDO_BASICO > 1000 AND EMP.EMAIL LIKE '%GMAIL.COM'
GO
...
SELECT
	...
	EMP.NIVEL_EDUCA,EMP.EMAIL, EMP.SUELDO_BASICO
FROM EMPLEADO AS EMP
WHERE EMP.ESTADO_CIVIL = 'S' 
	AND EMP.SUELDO_BASICO > 900 AND EMP.APELLIDOS LIKE '%E_'
GO
