...
-- 1. Mostrar los campos Num_Boleta, Id_cliente y el día en que se 
-- realizó la venta. Mostrar como nombre de columnas: 
-- Número Boleta, Código_cliente y Día de Venta. Utilizar Alias y función DAY.
SELECT
	BOL.NUM_BOLETA AS [Número Boleta],
	BOL.ID_CLIENTE AS Código_cliente,
	DAY(BOL.FECHA_EMI) AS 'Día de Venta'
FROM BOLETA AS BOL
GO

--2. Mostrar todos los campos de la tabla empleado, pero sólo aquellos 
--que hayan sido contratados(Fecha_ingreso) en el año 2015. Utilizar función YEAR.
SELECT *
FROM EMPLEADO AS EMP
WHERE YEAR(EMP.FECHA_INGRESO) = 2015

--3. Mostrar todos los campos de la tabla empleado, pero sólo aquellos 
--que hayan sido contratados (Fecha_ingreso) en el año 2014 y de estado civil 
--casado y nivel de educación superior.
SELECT *
FROM EMPLEADO AS EMP
WHERE YEAR(EMP.FECHA_INGRESO) = 2014
AND EMP.ESTADO_CIVIL = 'C'
AND EMP.NIVEL_EDUCA = 'SUPERIOR'


--4. Mostrar los tres primeros productos cuya categoría sea “C01” y año de 
-- vencimiento entre 2018 y 2020 del mes de agosto o mayo. Ordena por el campo 
-- Stock en coincidencia con el año de vencimiento en forma descendente.
SELECT TOP 1 *
FROM PRODUCTO AS PRO
WHERE PRO.COD_CATE = 'C01'
	AND YEAR(PRO.FECHA_VENC) BETWEEN 2018 AND 2020
	AND MONTH(PRO.FECHA_VENC) IN (8,5)
ORDER BY PRO.STOCK_ACTUAL DESC
GO

--5. Mostrar los campos Num_Boleta, Id_cliente, el día, el mes y el año 
--en que se realizó la venta por separado. Mostrar como nombre de columnas: 
--Número Boleta, Código Cliente, Día de Venta, mes de venta y Años de venta, 
--sólo aquellas ventas realizadas a partir del año 2014 y ordenados 
--de forma ascendente por código de cliente. 
--Utilizar Alias y funciones Day, Month y Year.
SELECT
	BOL.NUM_BOLETA AS 'Número Boleta',
	BOL.ID_CLIENTE AS 'Código Cliente',
	DAY(BOL.FECHA_EMI) AS 'Día de Venta',
	MONTH(BOL.FECHA_EMI) AS 'Mes de Venta',
	YEAR(BOL.FECHA_EMI) AS 'Año de venta'
FROM BOLETA AS BOL
WHERE
	YEAR(BOL.FECHA_EMI) >= 2014
ORDER BY BOL.ID_CLIENTE ASC
GO
...
SELECT
	--CONCAT(EMP.NOMBRES,' ',YEAR(EMP.FECHA_INGRESO)) AS EMPLEADO
	--EMP.NOMBRES +' ' +CAST(YEAR(EMP.FECHA_INGRESO) AS VARCHAR(4)) AS EMPLEADO
	EMP.NOMBRES+' '+EMP.APELLIDOS AS EMPLEADO,
	DATEDIFF(DD, EMP.FECHA_INGRESO, GETDATE()) AS DIAS,
	DATEDIFF(MM, EMP.FECHA_INGRESO, GETDATE()) AS MESES,
	DATEDIFF(YY, EMP.FECHA_INGRESO, GETDATE()) AS AÑOS,
	DATEDIFF(WW, EMP.FECHA_INGRESO, GETDATE()) AS SEMANAS
FROM EMPLEADO AS EMP

-- 7. Script que permita mostrar todos los valores correspondientes al argumento 
-- que presenta la función DATEPART.

-- 8. Mostrar los campos nombres, cantidad de años de permanencia, dirección y 
-- cantidad de días (antigüedad) de la tabla empleado.
